...
export function parseRatio(value) {
	if (value && typeof value === "object") {
		return {
			layout: String(value.layout || "1").trim() || "1",
			cells: String(value.cells || "1").trim() || "1",
			rotation: Number.isFinite(Number(value.rotation)) ? Math.round(Number(value.rotation)) : 0,
		};
	}
	...
	return {
		...
		rotation: Number.isFinite(Number(rotationPart)) ? Math.round(Number(rotationPart)) : 0,
	};
}

export function blankRatioRegion() {
	return {
		ratio: { layout: "1", cells: "1", rotation: 0 },
	};
}

export function ensureRatioProperties(node, activeCount = null) {
	const state = ensureRegionalColor(node);
	state.activeRegions = clampInt(activeCount ?? state.activeRegions ?? 1, 1, MAX_REGIONS);

	for (let i = 1; i <= MAX_REGIONS; i++) {
		const id = String(i);
		if (!state.regions[id] || typeof state.regions[id] !== "object") {
			state.regions[id] = blankRatioRegion();
		}
		state.regions[id].ratio = parseRatio(state.regions[id].ratio);
	}
	return state;
}
